...
public class SocialConnector
{
    ...
    [DllImport("__Internal")]
    private static extern void SocialConnector_Mail(string to, string title, string message, string logFileUrl);
#elif UNITY_ANDROID
    ...
    private static void _Share(string text, string url, string textureUrl)
    {
        using (var intent = new AndroidJavaObject("android.content.Intent"))
        {
            ...
            if (!string.IsNullOrEmpty(textureUrl))
            {
                ...
                var uri = fileProvider.CallStatic<AndroidJavaObject>("getUriForFile", currentActivity,
                    "studio.hexat.arika.fileprovider", file);
...
#if UNITY_EDITOR
#elif UNITY_IOS
        SocialConnector_Mail(to, title, message, logFileUrl);
#else
#endif
    }
